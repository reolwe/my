...
wbook = pxl.load_workbook('C:/Users/m.shcherbina/PycharmProjects/ITOIUItransfer/input.xlsx')
wSbook = wbook.active
#wSnobook = wbook.get
...
noreplacedict = {}
doubledict = {}
quntity = ('Картридж (лазерный)', 'Картридж (струйный)', 'Картридж', 'Сетевой фильтр', 'Клавиатура', 'FAX', 'Комплект (клавиатура+мышь)', 'Мышь', 'Патч-корд')
...
for i in range(1, wSbook.max_row + 1):
   for key in doubledict:
        if wSbook.cell(i, 9).value == key:
            new_name = doubledict[key]['name']
            new_type = doubledict[key]['type_ob']
            new_product = doubledict[key]['product']
            wSbook.cell(i, 8).value = str(new_name) + ', ' + str(wSbook.cell(i, 10).value) #формируем новое наименование из типа имущества и серийника
...
for i in range(1, wSbook.max_row + 1):
   for key in replacedict:
        if wSbook.cell(i, 8).value == key or wSbook.cell(i, 3).value == key:
...
print('выполнено', n, 'замен имущества, с дублем парт-номеров')
